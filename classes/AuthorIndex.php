...
/**
 * @file plugins/generic/recommendByAuthor/classes/AuthorIndex.php
 *
...
class AuthorIndex
{
    ...
    public function index(array $submissionIds): void
    {
        ...
        DB::transaction(function () use ($submissionIds, $entries) {
            foreach (array_chunk($submissionIds, 500) as $chunk) {
                DB::table('recommend_author_index')->whereIn('submission_id', $chunk)->delete();
            }
            foreach (array_chunk(array_values($entries), 500) as $chunk) {
                DB::table('recommend_author_index')->insert($chunk);
            }
        });
    }
    ...
    {
        $rows = DB::table('recommend_author_index as source')
            ...
            ->select('related.submission_id', DB::raw('COUNT(*) as shared'))
            ->get();
    ...
    public function relatedToMany(array $submissionIds): array
    {
        ...
        foreach (array_chunk($submissionIds, 50) as $chunk) {
            $rows = DB::table('recommend_author_index as source')
                ...
                ->select('source.submission_id', 'related.submission_id as related_id', DB::raw('COUNT(*) as shared'))
                ->get();
            foreach ($rows as $row) {
    ...
    public function neighboursOf(array $submissionIds): array
    {
        ...
        return DB::table('recommend_author_index as source')
            ->join('recommend_author_index as related', function ($join) {
                $join->on('related.author_key', '=', 'source.author_key')
                    ->on('related.context_id', '=', 'source.context_id');
            })
    ...
    private function keysOf(array $settings): array
    {
        ...
        foreach (array_keys($given + $family) as $locale) {
            if ($key = AuthorKey::fromName($given[$locale] ?? null, $family[$locale] ?? null)) {
                $keys[$key] = true;
            }
        }
        ...
        if ($this->matchByOrcid) {
            foreach ($settings['orcid'] ?? [] as $orcid) {
                if ($key = AuthorKey::fromOrcid($orcid)) {
                    $keys[$key] = true;
                }
            }
        }
