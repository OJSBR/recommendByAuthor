...
/**
 * @file plugins/generic/recommendByAuthor/classes/RecommendByAuthorStore.inc.php
 *
...
class RecommendByAuthorStore
{
    ...
    public function enqueueNew(array $contextIds, int $limit = 5000, int $queueLimit = 0): int
    {
        ...
        if ($queueLimit > 0) {
            $room = $queueLimit - Capsule::table('recommend_author_state')->count();
            if ($room <= 0) {
                return 0;
            }
        ...
        $missing = Capsule::table('submissions as s')
        ...
        foreach (array_chunk($rows, 500) as $chunk) {
            Capsule::table('recommend_author_state')->insertOrIgnore($chunk);
        }
    ...
    {
        Capsule::table('recommend_author_state')->insertOrIgnore([[
     ...
     */
    public function indexPending(RecommendByAuthorIndex $index, int $limit = 5000): int
    {
        $submissionIds = Capsule::table('recommend_author_state')
        ...
        foreach (array_chunk($submissionIds, 500) as $chunk) {
            Capsule::table('recommend_author_state')->whereIn('submission_id', $chunk)->update(['indexed_at' => now()]);
        }
    ...
    {
        return Capsule::table('recommend_author_state')->whereNull('indexed_at')->count();
    }
    ...
    {
        foreach (array_chunk($submissionIds, 500) as $chunk) {
            Capsule::table('recommend_author_state')->whereIn('submission_id', $chunk)->update(['indexed_at' => null]);
        }
    }
    ...
    public function due(array $contextIds, int $limit, int $maxAgeDays): array
    {
        ...
        $rows = Capsule::table('recommend_author_state')
     ...
     */
    public function refresh(array $submissionIds, RecommendByAuthorIndex $index, array $ranking, int $max): void
    {
        ...
        foreach (array_chunk($submissionIds, 50) as $chunk) {
            ...
            Capsule::connection()->transaction(function () use ($chunk, $rows) {
                Capsule::table('recommend_author_cache')->whereIn('submission_id', $chunk)->delete();
                foreach (array_chunk($rows, 500) as $insert) {
                    Capsule::table('recommend_author_cache')->insert($insert);
                }
                Capsule::table('recommend_author_state')
                    ->whereIn('submission_id', $chunk)
                    ->update(['indexed_at' => now(), 'computed_at' => now(), 'version' => Capsule::raw('version + 1')]);
            });
        }
    }
    ...
    public function ranking(int $contextId, string $orderBy, int $ttlHours = 12): array
    {
        ...
        // 3.3 never sets a Laravel facade root, so there is no Cache::remember
        // here. The file cache the rest of this release uses does the same job,
        // with one difference that matters: it has no notion of a TTL, so the
        // age is checked here and the file is dropped when it is too old.
        $cache = CacheManager::getManager()->getFileCache(
            'recommendByAuthor',
            'ranking-' . $contextId,
            function ($cache, $id) use ($contextId) {
                // The fallback has to write the cache, not just return: it is
                // getContents() that the caller reads, and that returns what
                // setEntireCache() stored.
                $ranking = $this->metricRanking($contextId);
                $cache->setEntireCache($ranking);
                return $ranking;
            }
        );

        $cachedAt = $cache->getCacheTime();
        if ($cachedAt !== null && (time() - $cachedAt) > max(1, $ttlHours) * 3600) {
            $cache->flush();
        }

        // An empty ranking is a real answer -- a journal with no statistics
        // yet -- and it caches as an empty array rather than as a miss, so it
     ...
     *
     * 3.4 split this into metrics_submission; in 3.3 it is still the single
     * metrics table, which also holds rows for issues and galleys, so the
     * metric type is pinned and rows with no submission are left out.
     */
    private function metricRanking(int $contextId): array
    {
        $rows = Capsule::table('metrics')
            ->where('context_id', $contextId)
            ->where('metric_type', METRIC_TYPE_COUNTER)
            ->whereIn('assoc_type', array(ASSOC_TYPE_SUBMISSION, ASSOC_TYPE_SUBMISSION_FILE))
            ->whereNotNull('submission_id')
            ->groupBy('submission_id')
            ->select('submission_id', Capsule::raw('SUM(metric) as value'))
            ->get();
    ...
    {
        return Capsule::table('recommend_author_cache')
    ...
    {
        return Capsule::table('recommend_author_cache')->where('submission_id', $submissionId)->count();
    }

    public function stateOf(int $submissionId): ?object
    {
        return Capsule::table('recommend_author_state')->where('submission_id', $submissionId)->first();
    }
